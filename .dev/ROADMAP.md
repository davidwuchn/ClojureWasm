...
| 6.16 | Tier A clojure.core primitive cluster (33 fns): type predicates in `core.zig` (string?/integer?/number?/symbol?/keyword?/vector?/list?/map?/set?/fn?/boolean?/char?/float?/ratio?/decimal?/some?/not/coll?/seq?/sequential?/associative? + identity/boolean) and arithmetic / sign in `math.zig` (inc/dec/zero?/pos?/neg?/odd?/even?/abs). Single-tag-test for predicates; delegation to `+`/`-`/`<`/`>` for arithmetic so the numeric promotion ladder applies unchanged. Underpins clojure.string and most clojure.core callers that pattern-match on type or shift by ±1 | [x] (`4bbbb9a`)        |
