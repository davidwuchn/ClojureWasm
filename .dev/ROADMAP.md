...
### 9.9 Phase 7 — task list (IN-PROGRESS; opened 2026-05-26)
...
| 7.2  | D-014c — multimethod dispatch + TypeDescriptor hierarchies (`defmulti` / `defmethod` + `prefer-method` / `derive`)                                                                                                                                                   | [ ]                                                              |
| 7.3  | D-014d — `defprotocol` satisfy + extend-type / extend-protocol. CallSite cache full activation                                                                                                                                                                       | [ ]                                                              |
| 7.4  | 5.12.b carry-forward — `defrecord` analyzer + eval. Implicit IPersistentMap semantics (get/assoc/keys/vals over field names). Uses Phase 7 dispatch ABI                                                                                                              | [ ]                                                              |
| 7.5  | 5.12.c carry-forward — `reify` analyzer + eval. Anonymous TypeDescriptor + closure capture + protocol-method bodies                                                                                                                                                  | [ ]                                                              |
| 7.6  | 5.12.d carry-forward — `(.method instance args)` general-arity protocol method dispatch via CallSite cache                                                                                                                                                           | [ ]                                                              |
| 7.7  | D-069 — Phase 6.16.a-1/a-2 polymorphic primitives (count / seq / conj / reduce) refactored to hybrid: Zig Tag-switch fast-path + Protocol extension point opens (= `extend-type` reaches them)                                                                       | [ ]                                                              |
| 7.8  | D-070 — multi-arity `fn*` / `defn` analyzer extension. `(fn* ([x] body1) ([x y] body2))` + arity dispatch table. Back-fills transducer 1-arg arity + multi-fn comp/juxt + multi-arg partial/complement/every? deferred at 6.16.a-3                                   | [ ]                                                              |
| 7.9  | D-072 — apply on lazy_seq tail integration via IReduce protocol (D-069 amendment). `(apply + (range))` early-termination semantics                                                                                                                                   | [ ]                                                              |
| 7.10 | D-073 — VM backend parity for has_rest + ns macro + require + libspec (`:as`/`:refer`). VM compileRequire/compileNs full surface                                                                                                                                     | [ ]                                                              |
| 7.11 | D-077 — catch class name → type tag dispatch table (replaces silent ExceptionInfo-only matching in tree_walk + VM)                                                                                                                                                  | [ ]                                                              |
| 7.12 | D-078 — clojure.string RED set Pattern A landing once `instance?` ships (replace / replace-first / escape). Closes D-062 named-exception list                                                                                                                        | [ ]                                                              |
| 7.13 | D-080 — clojure.zip Pattern A 28 vars (gated on Phase 7 deftype/defrecord landing). May land in a dedicated Phase 6.17 cycle if defrecord arrives mid-Phase-7                                                                                                        | [ ]                                                              |
| 7.14 | Phase 7 exit smoke — e2e: `(defprotocol P (m [x]))` + `(extend-type Foo P (m [x] ...))` + `(.m foo-instance)` runs the user impl; `(defmulti area :shape)` + `(defmethod area :circle [x] ...)` dispatches correctly                                                 | [ ]                                                              |
| 7.15 | Final activation step — flip `build_options.phase_at_least_7 = true` (per ADR-0023) — swaps `runtime/protocol/stub.zig` → real dispatch                                                                                                                            | [ ]                                                              |
