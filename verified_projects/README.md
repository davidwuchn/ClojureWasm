...
cljw**, proven by a self-contained `deps.edn` (git-coordinate resolution) plus a
`verify.clj` exercise. The directory list is the at-a-glance answer to "which
...
verified_projects/
  <lib>/
    deps.edn     pins the lib via :git/url + :git/sha (a real GitHub coord)
    verify.clj   (require …) + asserts known-good outputs + prints "OK <lib>"
```

The dir name is the human label (e.g. `medley`, `data.priority-map`). The
`deps.edn` is exactly what a user would write to depend on the lib; cljw resolves
it source-only (`:git/url`/`:local/root`; `:mvn/version` is skipped per ADR-0101
amendment 1 — `org.clojure/clojure` is cw itself, other coords resolve at
`require` time against cw's bundled namespaces). `verify.clj` must `assert` real
outputs (not just `require`) and end with `(println "OK <lib>")` so the sweep can
report a one-line result.

## Run

```sh
bash scripts/verify_projects.sh            # all projects
...
2. `mkdir verified_projects/<lib>` with `deps.edn` (`:git/url`+`:git/sha`) and
   `verify.clj` (require + asserts + `OK` line).
3. `bash scripts/verify_projects.sh <lib>` — if it passes, commit the dir.
