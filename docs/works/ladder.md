...
| 4b   | flatland.ordered (amalloy) | master        | 1           | fails      | Probed 2026-06-07 (deftype-heavy, post-priority-map). Fails at set.clj:67 bare `IHashEq` — the `(:import (clojure.lang …))` + bare-name deftype-supertype pattern (vs priority-map's qualified names) → D-286: (a) bare host-interface aliases, (b) bare-modeled-name protocol_remap routing.                                                                                                           |
