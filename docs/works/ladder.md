...
| 4    | clojure.data.priority-map  | master        | 1           | loads      | `(require 'clojure.data.priority-map)` SUCCEEDS (D-275 + D-279 + D-280 clojure.lang.* family + D-281 java.util.Map/Iterable host_inert + D-282 clojure.core.protocols/IKVReduce). Every deftype supertype resolves; method bodies are def-time-unevaluated so don't block load. NOT yet fully functional: `(priority-map …)` construction calls `.assoc`/`.valAt` (clj-name dot-calls) which protocol_remap translated to -assoc/-lookup → D-283 (register both names). |
