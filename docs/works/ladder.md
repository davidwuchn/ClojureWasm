...
| 6    | clojure.tools.reader       | master        | 2           | fails      | Re-probed 2026-06-07 after D-288 landed: StringReader's `^:unsynchronized-mutable ^long s-pos` + set!/update! now parses (D-288 done). Advances to reader_types.clj:62 `aget` → **D-287 (Java arrays)** — InputStreamReader (`^"[B"` byte-array + aget/byte-array/.read/Closeable) + PushbackReader (`^"[Ljava.lang.Object;"` object-array + aget) force it even though StringReader alone is now clean.                          |
