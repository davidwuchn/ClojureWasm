...
| 5    | clojure.data.generators    | master        | 2           | fails      | Re-probed 2026-06-07 after D-295 (Short/Byte static fields): generators.clj advances :110 Short -> :149 `vec` -> D-296 (a macro-expansion/analysis-order resolution issue: the (primitive-arrays [...]) call at :163 expands defns; bare `vec` unresolved at the primitive-array template :149 though the file defines vec at :187 and plain syntax-quote-exclude-forward-ref works). Cleared so far: D-289 (Random), D-295 (Short/Byte). The Random/test.check enablement arc is deep. |
