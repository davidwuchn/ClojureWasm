...
;; cljw: `*warn-on-reflection*` is a no-op JVM reflection directive (cljw does
;; no reflective interop), so the top-level `(set! *warn-on-reflection* true)`
;; is dropped — an eager-bundled lib loads before the baseline binding frame is
;; active, so a top-level set! of a config var would raise (latent gap D-477).
